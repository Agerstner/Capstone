package com.claim_academy.capstone;
...
public class WebUtils {
	...
	public void addMaintenancePhoto(MultipartFile file, long id, String folder) throws IllegalStateException, IOException {
		
		try {
			// save dir
			String destDir = request.getSession().getServletContext().getRealPath(UPLOADED_FOLDER) + File.separator
					+ folder + File.separator + id + File.separator + "profile maintenance requests";
			...
			// rename file
			File rename = new File(destDir + File.separator + "maint_" + id + "maintRequest" + ".jpg");
			doc.renameTo(rename);
		
			// save to file system
			multipartFile.transferTo(rename);
			MaintenanceReports report = maintenanceRepository.findById(id).get();
			report.setImage("user_" + id + "maintRequest" + ".jpg");
	...
	public void multipleMaintPhotoSave(MultipartFile[] files, long id) throws IllegalStateException, IOException {

			String realPathtoUploads = request.getSession().getServletContext()
					.getRealPath(UPLOADED_FOLDER + File.separator + "reports");
	...
	public void multiplesave(MultipartFile[] files, long id) throws IllegalStateException, IOException {
		...
		try {
			String realPathtoUploads = request.getSession().getServletContext()
					.getRealPath(UPLOADED_FOLDER + File.separator + "users");
			File dir = new File(realPathtoUploads + File.separator + id);
			List<String> fileNames = new ArrayList<String>();
